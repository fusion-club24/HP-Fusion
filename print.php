...
if ((isset($_GET['type']) && $_GET['type'] == "A") && (isset($_GET['item_id']) && isnum($_GET['item_id']))) {
	...
	if (dbrows($result)) {
		...
		if (checkgroup($data['article_cat_access'])) {
			$res = true;
			$article = str_replace("<--PAGEBREAK-->", "", stripslashes(nl2br(parsesmileys(parseubb($data['article_article'])))));
			if ($data['article_breaks'] == "y") { $article = stripslashes(nl2br(parsesmileys(parseubb($article)))); }
	...
	if (dbrows($result) != 0) {
		...
		if (checkgroup($data['news_visibility'])) {
			$res = true;
			$news = stripslashes(nl2br(parsesmileys(parseubb($data['news_news']))));
			if ($data['news_breaks'] == "y") { $news = nl2br($news); }
			if ($data['news_extended']) {
				$news_extended = stripslashes(nl2br(parsesmileys(parseubb($data['news_extended']))));
				if ($data['news_breaks'] == "y") { $news_extended = $news_extended; }
			} else {
				$news_extended = "";
			}
	...
	echo "<hr />".nl2br(parseubb(parsesmileys($settings['license_agreement'])))."\n";
} else {
	redirect("index.php");
}
echo "</body>\n</html>\n";
