...
define("THEME_BULLET", "<img src='".DESIGN."images/bullet.gif' class='bullet' alt='&raquo;' border='0' />");
define("THEME_WIDTH", "1140px");

$enable_colour_switcher = true;

require_once DESIGN."functions.php";
require_once DESIGNS."templates/switcher.php";
...
function render_page($license = false) {
	...
	add_to_head("<link rel='stylesheet' type='text/css' href='".DESIGN."styles_add.css' />");
	...
	#echo "<div class='sub-header clearfix floatfix'>".rendersublinks("", "")."</div>\n";
	echo "<div>".render_menu()."</div>\n";
	...
	echo "<br />Theme designed by <a href='http://www.agxthemes.com'>Falcon</a> darken by SuNflOw1991</div>\n";
	echo "<div class='flright' style='width: 50%; text-align: right;'>".stripslashes($settings['footer'])."<br />";
	echo "<a href='".BASEDIR."contact.php'>Kontakt</a> | <a href='".BASEDIR."site.php?site_id=2'>Datenschutz</a> | <a href='".BASEDIR."site.php?site_id=1'>Impressum</a>\n";
	echo "</div>\n";
	echo "</div>\n";
...
function rendersublinks($sep = "&middot;", $class = "")
{
	global $userdata;
	...
	if (dbrows($sres)) {
		...
		$li_class = "";
		if (START_PAGE == "login.php") {
			$li_class = "current-link";
		}
		$res .= "<li".($li_class != "" ? " class='".$li_class."'" : "")." style='float:right;'>\n";
		$res .= "<a href='".BASEDIR."login.php'>";
		if (!iMEMBER) {
			$res .= "<span>Login / Register</span>";
		} else {
			$res .= "<span>".$userdata['user_name']."</span>";
		}
		$res .= "</a>";
		$res .= "</li>\n";
...
function render_news($subject, $news, $info)
{
	...
	set_image("edit", DESIGN."images/icons/news_edit.png");
	...
	if ($info['news_sticky'] == "1") {
		echo "<div style='position:absolute; padding-top:3px;'><img src='".DESIGN."images/icons/sticky.png' alt='sticky' width='41px' border='0' height='41px' /></div>";
	}
...
function render_article($subject, $article, $info)
{
	...
	set_image("edit", DESIGN."images/icons/article_edit.png");
