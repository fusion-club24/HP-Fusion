...
if (iMEMBER) {
	$result = dbquery("UPDATE ".DB_USERS." SET user_lastvisit='".time()."', user_ip='".USER_IP."' WHERE user_id='".$userdata['user_id']."'");
}
...
echo "<style type=\"text/css\">
	body { font-family:Verdana,Tahoma,Arial,Sans-Serif;font-size:14px;background-color: #ffffff; }
	hr { height:1px;color:#ccc; }
	td { border : 1px dotted #ddd; }
	a { color: #5c6d15;background: inherit;text-decoration:none; }
	a:hover { background: inherit;text-decoration:none; }
	a.side { color: #5c6d15;background: inherit;text-decoration:none; }
	a:hover.side { background: inherit;text-decoration:none; }
	img { border: 0; }
...
if (!isset($_GET['msg_send']) && !isset($_GET['msg_read']) && $_GET['folder'] != "options") {
	...
	if (dbrows($result)) {
		...
		add_to_title($locale['global_201'].$locale['431']);
                echo "<div align='right' ><a href='JavaScript:window.print();'><img src='".get_image("printer")."' alt='".$locale['global_075']."' style='vertical-align:middle;border:0;' />".$locale['global_075']."</a>&nbsp;</div>"; 
		...
		echo "<table cellpadding='0' cellspacing='1' width='100%' class='tbl-border'>\n<tr>\n";
		echo "<td align='left' width='1%' class='tbl2' style='white-space:nowrap'>".($_GET['folder'] != "outbox" ? $locale['406'] : $locale['421'])."</td>\n";
		echo "<td class='tbl1'><a href='".BASEDIR."profile.php?lookup=".$data['user_id']."'>".$data['user_name']."</a></td>\n</tr>\n";
		echo "<tr>\n<td align='left' width='1%' class='tbl2' style='white-space:nowrap'>".$locale['407']."</td>\n";
		echo "<td class='tbl1'>".showdate("longdate", $data['message_datestamp'])."</td>\n</tr>\n";
		echo "<tr>\n<td align='left' width='1%' class='tbl2' style='white-space:nowrap'>".$locale['405']."</td>\n";
	...
	if (!isset($_POST['chk_sendtoall']) || $_GET['msg_send'] != "0") {
		...
		$result = dbquery("SELECT u.user_id, u.user_name FROM ".DB_USERS." u ORDER BY user_level DESC, user_name ASC");
		while ($data = dbarray($result)) {
			if ($data['user_id'] != $userdata['user_id']) {
	...
	if ($_GET['msg_send'] == "0") {
		...
		$udata = dbarray(dbquery("SELECT u.user_id, u.user_name FROM ".DB_USERS." u WHERE user_id='".$_GET['msg_send']."'"));
