...
if (isset($_GET['article_id']) && isnum($_GET['article_id'])) {
	...
	if (dbrows($result)) {
		...
		$article = preg_split("/<!?--\s*pagebreak\s*-->/i", (stripslashes(parsesmileys(parseubb($data['article_article'])))));
	...
	if (dbrows($result) != 0) {
		...
		if (checkgroup($cdata['article_cat_access'])) {
			...
			if ($rows != 0) {
				$result = dbquery(
					"SELECT article_id, article_subject, article_snippet, article_breaks, article_datestamp FROM ".DB_ARTICLES."
				...
				while ($data = dbarray($result)) {
					...
					echo "<div class='".$class."'><strong><a href='".FUSION_SELF."?article_id=".$data['article_id']."'>".$data['article_subject']."</a></strong>".$new."<br />\n".preg_replace("/<!?--\s*pagebreak\s*-->/i", "", 
					($data['article_breaks'] == "y" ? stripslashes(nl2br(parsesmileys(parseubb($data['article_snippet'])))) : stripslashes(parsesmileys(parseubb($data['article_snippet'])))))."<div class='clear'></div></div>";
					echo ($i != $numrows ? "<hr />\n" : "\n"); $i++;
				}
