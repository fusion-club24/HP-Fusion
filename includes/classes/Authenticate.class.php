...
class Authenticate {
	...
	private function _authenticate($inputUserName, $inputPassword, $remember) {
		...
		//Login with Nickname or Email or both
		if (isset($settings['login_type']) &&  $settings['login_type']=='0') {
			$result = dbquery("   SELECT * FROM ".DB_USERS." WHERE user_name='".$inputUserName."' or user_email='".$inputUserName."' LIMIT 1");
