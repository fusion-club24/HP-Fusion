...
class UserFields {
	...
	public function displayInput() {
		...
		$this->html .= "<table cellpadding='0' cellspacing='1' class='center edit-profile'>\n";
	...
	private function renderValidation() {
		...
		if (!$_CAPTCHA_HIDE_INPUT) {
			...
			$this->html .= "<input type='text' id='captcha_code' name='captcha_code' class='textbox' style='width:100px' />";
		}
		$this->html .= "</td>\n</tr>\n";
	}
	...
	private function basicInputField($name, $text, $length, $isRequired = "", $type = "text", $haveValue = true, $error_class = "") {
		...
		$returnHTML = "<tr>\n";
		$returnHTML .= "<td valign='top' class='".$class."' width='150'><label for='".$name."'>".$text.$required."</label></td>\n";
		$returnHTML .= "<td class='".$class."'>";
		if ($name == "user_password")
		{
			$returnHTML .= "<input type='".$type."' id='".$name."' name='".$name."' maxlength='".$length."' class='textbox' value='".$value."' style='width:200px;' /><br />\n";
			} else {
				if ($name == "user_new_password" )
				{
		$returnHTML .= "<input type='".$type."' id='".$name."' name='".$name."' maxlength='".$length."' class='textbox' value='".$value."' onkeyup='P91PWS(this.value)' style='width:200px;' /><br />\n<div class='smallfont' id='P91PWS'>
			<div class='P91PWS_O'>Nicht bewertet</div>
			<div class='P91PWS_C'></div></div>";
				} else {
	...
	private function renderBasicInputFields() {
		...
		if (strstr(FUSION_REQUEST, "register"))
		{
		$this->html .= $this->basicInputField("user_new_password", $locale['u133'], "64", $passRequired, "password", false, "user_password");
	...
	private function basicOutputField($name, $value, $class, $rowspan = 0) {
		...
		$returnHTML = "<tr>\n";
		if ($rowspan > 0) {
			$returnHTML .= "<td rowspan='".$rowspan."' valign='top' class='tbl profile_user_avatar' width='120'><!--profile_user_avatar-->";
			$returnHTML .= "<img src='".IMAGES."avatars/".$this->userData['user_avatar']."' class='avatar' alt='".$locale['u062']."' title='".$locale['u062']."' />";
			$returnHTML .= "</td>\n";
		}
		$returnHTML .= "<td align='left' class='tbl1'>".$name.":</td>\n";
		$returnHTML .= "<td align='left' class='".$class." tbl1'><!--".$class."-->".$value."</td>\n";
		$returnHTML .= "</tr>\n";
	...
	private function renderBasicOutputFields() {
		...
		$this->html .= "<table cellpadding='0' cellspacing='1' width='100%' class='profile tbl-border center'>\n";
		...
		$lastVisit = $this->userData['user_lastvisit'] ? showdate("longdate", $this->userData['user_lastvisit'])." Uhr" : $locale['u042'];
		$returnFields .= $this->basicOutputField($locale['u066'], showdate("longdate", $this->userData['user_joined'])." Uhr", "profile_user_joined");
	...
	private function renderAdminOptions() {
		...
		$this->html .= "<table cellpadding='0' cellspacing='0' width='100%' class='profile tbl-border center'>\n<tr>\n";
		$this->html .= "<td class='tbl2'><strong>".$locale['u058']."</strong></td>\n";
		$this->html .= "</tr>\n<tr>\n";
		$this->html .= "<td style='line-height: 20px;' align='center'><!--profile_admin_options-->\n";
		...
		if (!is_null($groups_cache) && count($groups_cache) > 0){
			...
			if (iADMIN && checkrights("UG") && $user_groups_opts) {
				$this->html .= "</tr>\n<tr><td style='line-height: 28px' align='center'>".$locale['u061'].":\n";
	...
	private function renderAvatarInput() {
		...
		$this->html .= "<input type='file' id='user_avatar_upload' name='user_avatar' class='file' /><br />\n";
		...
		foreach ($cats as $cat) {
			if (array_key_exists($cat['field_cat'], $fields) && $fields[$cat['field_cat']]) {
				...
				if ($this->method == "display") {
					$this->html .= "<div style='margin:5px'></div>\n";
					$this->html .= "<table cellpadding='0' cellspacing='1' width='100%' class='profile_category tbl-border center'>\n";
				}
				$this->html .= "<tr>\n";
