...
$result = dbquery(
	"SELECT td.download_id, td.download_title, td.download_count, td.download_cat, tc.download_cat_id,
	tc.download_cat_access
	...
	if (dbrows($cat_result)) {
		...
		while($cat_data = dbarray($cat_result)) {
			...
			if (checkgroup($cat_data['download_cat_access'])) {
				...
				if ($rows != 0) {
					$result = dbquery(
						"SELECT td.download_id, td.download_user, td.download_datestamp, td.download_image_thumb, td.download_image, td.download_cat,
					...
					while ($data = dbarray($result)) {
						...
						echo "<tr>\n";
							...
							echo "<td class='tbl2' style='text-align:left;'><a href='".FUSION_SELF."?cat_id=".$cat_data['download_cat_id']."&amp;download_id=".$data['download_id']."'>".$data['download_title']."</a></td>\n";
						...
						echo "<tr>\n";
							echo "<td colspan='8' class='tbl1 small'>\n";
								if ($settings['download_screenshot']) {
									if (isset($data['download_image']) && $data['download_image'] != "") {
										echo "<a class='tozoom' href='".DOWNLOADS."images/".$data['download_image']."'><img src='".$img_thumb."' style='float: left;margin:3px;' alt='".$data['download_title']."' /></a>\n";
									} else {
...
if (isset($_GET['download_id']) && isnum($_GET['download_id'])) {
	...
	if (dbrows($result)) {
		...
		echo "<tr>\n";
			echo "<td class='tbl1' style='vertical-align:top;'>".($data['download_description'] != "" ? nl2br(parseubb(parsesmileys($data['download_description']))) : nl2br(stripslashes($data['download_description_short'])))."</td>";
