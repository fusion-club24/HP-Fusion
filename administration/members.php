...
// Delete User
	...
	if (isset($_POST['delete_user'])) {
		...
		opentable($locale['410']." ".$locale['612'].": ".$user_data['user_name']);
			...
			echo sprintf($locale['425a'], $user_data['user_name']);
			}
	...
	if (dbrows($result)) {
		...
		if (isset($_POST['ban_user'])) {
			if ($udata['user_status'] == 1) {
				...
				$message = str_replace("[USER_NAME]", $udata['user_name'], $locale['email_unban_message']);
				sendemail($udata['user_name'], $udata['user_email'], $settings['siteusername'], $settings['siteemail'], $locale['email_unban_subject'], $message);
	...
	if (dbrows($result)) {
		...
		if (isset($_POST['suspend_user'])) {
			if ($udata['user_status'] == 3) {
				...
				$message = str_replace("[USER_NAME]", $udata['user_name'], $locale['email_unsuspend_message']);
				sendemail($udata['user_name'], $udata['user_email'], $settings['siteusername'], $settings['siteemail'], $locale['email_unsuspend_subject'], $message);
	...
	if (dbrows($result)) {
		...
		if (isset($_POST['sban_user'])) {
			if ($udata['user_status'] == 4) {
				...
				$message = str_replace("[USER_NAME]", $udata['user_name'], $locale['email_unsecban_message']);
				sendemail($udata['user_name'], $udata['user_email'], $settings['siteusername'], $settings['siteemail'], $locale['email_unsecban_subject'], $message);
