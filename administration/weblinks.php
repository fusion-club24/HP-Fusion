...
if (!empty($result)) {
	if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['weblink_id']) && isnum($_GET['weblink_id']))) {
		$result = dbquery("SELECT weblink_url FROM ".DB_WEBLINKS." WHERE weblink_id='".$_GET['weblink_id']."'");
			$data = dbarray($result);
			$img_name = str_replace(array('https:', 'http:', 'www', '/', '.'),array('', '', '', ''),$data['weblink_url']);
			if (file_exists(IMAGES."weblink_sites/".$img_name.".jpg")) {
				@unlink(IMAGES."weblink_sites/".$img_name.".jpg");
			}
	...
	echo "</tr>\n<tr>\n";
	echo "<td width='80' class='tbl'>".$locale['526']."</td>\n";
	echo "<td align='right' class='tbl' style='width:380px;'><a class='button' href='".ADMIN."screenshot.php".$aidlink."' target='_blank'>".$locale['527']."</a></td>\n";
	echo "</tr>\n<tr>\n";
