...
if (isset($_POST['delete_watermarks'])) {
	...
	if (!$result) { $error = 1; }
	if ($_POST['photo_max_b'] <= $upload_max_filesize) {
	...
	if (!$result) { $error = 1; }
	$result = dbquery("UPDATE ".DB_SETTINGS." SET settings_value='".($_POST['photo_closed'] == 0 || $_POST['photo_closed'] == 1 ? $_POST['photo_closed'] : "0")."' WHERE settings_name='photo_closed'");
	if (!$result) { $error = 1; }
	$result = dbquery("UPDATE ".DB_SETTINGS." SET settings_value='".stripinput($_POST['photo_closed_reason'])."' WHERE settings_name='photo_closed_reason'");
	if (!$result) { $error = 1; }
...
echo "<td width='50%' class='tbl'>".$locale['942']." ".$upload_max_filesize." = ".parsebytesize($upload_max_filesize)."<br /><input type='text' name='photo_max_b' value='".$settings2['photo_max_b']."' maxlength='10' class='textbox' style='width:100px;' /> = ".parsebytesize($settings2['photo_max_b'])."</td>\n";
echo "</tr>\n<tr>\n";
...
echo "<td width='50%' class='tbl'>".$locale['621']."<br /></td>\n";
echo "<td width='50%' class='tbl'><select name='photo_closed' size='1' class='textbox' style='width:100px;'>";
echo "<option value='1' ".($settings['photo_closed'] ? "selected='selected'" : "").">".$locale['518']."</option>\n";
echo "<option value='0' ".(!$settings['photo_closed'] ? "selected='selected'" : "").">".$locale['519']."</option>\n";
echo "</select></td>\n";
echo "</tr>\n<tr>\n";
echo "<td width='50%' class='tbl' valign='top'>".$locale['622']."<br /><span class='small2'>".$locale['623']."</span></td>\n";
echo "<td width='50%' class='tbl' valign='top'><textarea name='photo_closed_reason' cols='50' rows='6' class='textbox' style='width:230px;'>".$settings2['photo_closed_reason']."</textarea></td>\n";
echo "</tr>\n<tr>\n";
echo "<td width='50%' class='tbl'></td><td width='50%' class='tbl'>\n";
echo display_bbcodes("100%", "photo_closed_reason", "settingsform")."</td>\n";
echo "</tr>\n<tr>\n";
