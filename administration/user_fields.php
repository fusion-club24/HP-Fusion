...
elseif(isset($_GET['delete_field']) && file_exists(INCLUDES."user_fields/".stripinput($_GET['delete_field'])."_include_var.php")) {
	if(isset($_GET['delete_field'])) {
		if (file_exists(INCLUDES."user_fields/".stripinput($_GET['delete_field'])."_include_var.php")) {
			@unlink(INCLUDES."user_fields/".stripinput($_GET['delete_field'])."_include_var.php");
		}
		if (file_exists(INCLUDES."user_fields/".stripinput($_GET['delete_field'])."_include.php")) {
			@unlink(INCLUDES."user_fields/".stripinput($_GET['delete_field'])."_include.php");
		}
		if (file_exists(LOCALE."English/user_fields/".stripinput($_GET['delete_field']).".php")) {
			@unlink(LOCALE."English/user_fields/".stripinput($_GET['delete_field']).".php");
		}
		if (file_exists(LOCALE."German/user_fields/".stripinput($_GET['delete_field']).".php")) {
			@unlink(LOCALE."German/user_fields/".stripinput($_GET['delete_field']).".php");
		}
...
if (dbrows($result)) {
	...
	while($data = dbarray($result)) {
		...
		echo "<a href='".FUSION_SELF.$aidlink."&amp;enable=".$data['field_name']."'><img src='".IMAGES."edit.png' alt='".$locale['407']."' title='".$locale['407']."' style='border:0px;' /></a> -\n";
		echo "<a onclick = \"return confirm('".$locale['410']."');\" href='".FUSION_SELF.$aidlink."&amp;disable=".$data['field_id']."'><img src='".IMAGES."no.png' alt='".$locale['408']."' title='".$locale['408']."' style='border:0px;' /></a>\n";
		echo "</td>\n</tr>\n";
	}
} else {
...
if (count($available_fields) != count($enabled_fields)) {
	...
	for ($k = 0; $k < count($available_fields); $k++) {
		if (!in_array($available_fields[$k], $enabled_fields)) {
			...
			echo "<td width='1%' class='tbl1' style='white-space:nowrap'>".$user_field_name."</td>\n";
			echo "<td class='tbl1' style='white-space:nowrap'>".$user_field_desc."</td>\n";
			echo "<td width='1%' class='tbl1' style='white-space:nowrap'><a href='".FUSION_SELF.$aidlink."&amp;enable=".$available_fields[$k]."'>".$locale['431']."</a> - <a href='".FUSION_SELF.$aidlink."&amp;delete_field=".$available_fields[$k]."' onclick=\"return confirm('".$locale['433']."');\" class='side'>".$locale['432']."</a></td>\n";
			echo "</tr>\n";
			$i++;
		}
	}
