	...
	if (isset($_POST['cancel'])) {
		...
		if (!SAFEMODE) {
			...
			if (dbrows($result2)) {
				...
				$file_name2 = explode(".", $old_image);
				$new_w1 = $file_name2[0]."_w1.".$file_name2[1];
				$new_w2 = $file_name2[0]."_w2.".$file_name2[1];
				if (file_exists(PHOTODIR.$new_w1)) @rename (PHOTODIR.$new_w1, PHOTOS."album_".$_POST['move_album_id']."/".$new_w1);
				if (file_exists(PHOTODIR.$new_w2)) @rename (PHOTODIR.$new_w2, PHOTOS."album_".$_POST['move_album_id']."/".$new_w2);
		...
		if ($rows) {
			while ($data = dbarray($result)) {
				if (!SAFEMODE) {
					...
					unset($file_name);
					if ($data['photo_filename']) @rename (PHOTODIR.$old_image, PHOTOS."album_".$_POST['move_album_id']."/".$new_image);
				if (file_exists(PHOTODIR.$old_thumb1)) @rename (PHOTODIR.$old_thumb1, PHOTOS."album_".$_POST['move_album_id']."/".$new_thumb1);
				if (file_exists(PHOTODIR.$old_thumb2)) @rename (PHOTODIR.$old_thumb2, PHOTOS."album_".$_POST['move_album_id']."/".$new_thumb2);
					if ($old_image != $new_image) {
						$result2 = dbquery("UPDATE ".DB_PHOTOS." SET album_id='".$_POST['move_album_id']."', photo_order='".$last_order."', photo_filename='".$new_image."', photo_thumb1='".$new_thumb1."', photo_thumb2='".$new_thumb2."' WHERE photo_id='".$data['photo_id']."'");
					} else {
						$result2 = dbquery("UPDATE ".DB_PHOTOS." SET album_id='".$_POST['move_album_id']."', photo_order='".$last_order."' WHERE photo_id='".$data['photo_id']."'");
		...
		$data = dbarray(dbquery("SELECT photo_filename,photo_thumb1,photo_thumb2,photo_order FROM ".DB_PHOTOS." WHERE photo_id='".$_GET['photo_id']."'"));
		...
		$wm_file2 = $parts[0]."_w2.".$parts[1];
			...
			@unlink(PHOTODIR.$wm_file1);
			}
			if (file_exists(PHOTODIR.$wm_file2)) {
			@unlink(PHOTODIR.$wm_file2);
			}
		@unlink(PHOTODIR.$data['photo_filename']);
		@unlink(PHOTODIR.$data['photo_thumb1']);
		if (isset($data['photo_thumb2']) && $data['photo_thumb2']!='') { @unlink(PHOTODIR.$data['photo_thumb2']); }
		$result = dbquery("UPDATE ".DB_PHOTOS." SET photo_order=(photo_order-1) WHERE photo_order>'".$data['photo_order']."' AND photo_id='".$_GET['photo_id']."'");
		...
		if (file_exists(PHOTODIR.$data['photo_filename'])) { @unlink(PHOTODIR.$data['photo_filename']); }
		if (file_exists(PHOTODIR.$data['photo_thumb1'])) { @unlink(PHOTODIR.$data['photo_thumb1']); }
		if (file_exists(PHOTODIR.$data['photo_thumb2'])) { @unlink(PHOTODIR.$data['photo_thumb2']); }
