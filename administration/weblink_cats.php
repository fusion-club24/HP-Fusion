...
if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
	...
	if (isset($_POST['save_cat'])) {
		...
		if ($cat_name) {
			if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
				$result = dbquery("UPDATE ".DB_WEBLINK_CATS." SET weblink_cat_name='$cat_name', weblink_cat_image='$cat_image', weblink_cat_description='$cat_description', weblink_cat_sorting='$cat_sorting', weblink_cat_access='$cat_access' WHERE weblink_cat_id='".$_GET['cat_id']."'");
				redirect(FUSION_SELF.$aidlink."&status=su");
			} else {
				$checkCat = dbcount("(weblink_cat_id)", DB_WEBLINK_CATS, "weblink_cat_name='".$cat_name."'");
				if ($checkCat == 0) {
					$result = dbquery("INSERT INTO ".DB_WEBLINK_CATS." (weblink_cat_name, weblink_cat_image, weblink_cat_description, weblink_cat_sorting, weblink_cat_access) VALUES ('$cat_name', '$cat_image', '$cat_description', '$cat_sorting', '$cat_access')");
	...
	if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['cat_id']) && isnum($_GET['cat_id']))) {
		$result = dbquery("SELECT weblink_cat_name, weblink_cat_image, weblink_cat_description, weblink_cat_sorting, weblink_cat_access FROM ".DB_WEBLINK_CATS." WHERE weblink_cat_id='".$_GET['cat_id']."'");
		if (dbrows($result)) {
			$data = dbarray($result);
	...
	$catimage = makefileopts(makefilelist(IMAGES."weblink_cats/", ".|..|index.php|Thumbs.db"), $cat_image);
	...
	echo "</tr>\n<tr>\n";
	echo "<td width='1%' class='tbl' style='white-space:nowrap'>".$locale['418']."</td>";
    echo "<td class='tbl'><select name='cat_image' class='textbox' style='width:150px;'>".$catimage."</select></td>";
   	echo "</tr><tr>";
   	echo "<td colspan='2' class='tbl'>";
   	echo "<strong>".$locale['419']."</strong><br />";
   	$directory = "".IMAGES."weblink_cats";
      ...
      $imagetype= array("jpg", "jpeg", "gif", "png");
         ...
         echo "<div class='catgalerie'><img src='".$fileinfo['dirname']."/".$fileinfo['basename']."' alt=".$fileinfo['basename']."' title='".$fileinfo['basename']."' /><br /><center><span>".$fileinfo['basename']."</span></center></div>";
         }
      };
   	};
   	echo "</td>\n";
