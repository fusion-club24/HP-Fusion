...
if ((isset($_GET['action']) && $_GET['action'] == "2") && (isset($_GET['t']) && $_GET['t'] == "l")) {
	if (isset($_POST['add']) && (isset($_GET['submit_id']) && isnum($_GET['submit_id']))) {
		...
		if (dbrows($result)) {
			...
			echo "<td class='tbl'><textarea name='link_description' cols='50' rows='15' class='textbox' style='width:300px'>".$submit_criteria['link_description']."</textarea></td>\n";
			echo "</tr>\n</table>\n";
...
if ((isset($_GET['action']) && $_GET['action'] == "2") && (isset($_GET['t']) && $_GET['t'] == "n")) {
	if (isset($_POST['publish']) && (isset($_GET['submit_id']) && isnum($_GET['submit_id']))) {
		...
		if (dbrows($result)) {
			...
			$result = dbquery("INSERT INTO ".DB_NEWS." (news_subject, news_cat, news_news, news_extended, news_breaks, news_name, news_datestamp, news_start, news_end, news_visibility, news_reads, news_allow_comments, news_allow_ratings) VALUES ('".$news_subject."', '".$news_cat."', '".$news_snippet."', '".$news_body."', 'y', '".$data['user_id']."', '".time()."', '0', '0', '0', '0', '1', '1')");
			$result = dbquery("DELETE FROM ".DB_SUBMISSIONS." WHERE submit_id='".$_GET['submit_id']."'");
		...
		if (dbrows($result)) {
			...
			if (isset($_POST['preview']) && (isset($_GET['submit_id']) && isnum($_GET['submit_id']))) {
				$news_subject = stripinput($_POST['news_subject']);
				$news_cat = isnum($_POST['news_cat']) ? $_POST['news_cat'] : "0";
				$news_snippet = nl2br(parsesmileys(parseubb($_POST['news_snippet'])));
				$news_body = nl2br(parsesmileys(parseubb($_POST['news_body'])));
			...
			echo "<td width='90%' class='tbl'><textarea name='news_snippet' cols='60' rows='10' class='textbox' style='width:80%;'>".$news_snippet."</textarea></td>\n";
			echo "</tr>\n";
			echo "<tr>\n<td class='tbl'></td>\n<td class='tbl'>\n";
				echo display_bbcodes("100%", "news_snippet", "publish");
			echo "</td>\n</tr>\n";
			echo "<tr>\n";
			echo "<td valign='top' width='100' class='tbl'>".$locale['508']."</td>\n";
			echo "<td width='90%' class='tbl'><textarea name='news_body' cols='60' rows='10' class='textbox' style='width:80%;'>".$news_body."</textarea></td>\n";
			echo "</tr>\n";
			echo "<tr>\n<td class='tbl'></td>\n<td class='tbl'>\n";
				echo display_bbcodes("100%", "news_body", "publish");
			echo "</td>\n</tr>\n";
			echo "<tr>\n";
...
if ((isset($_GET['action']) && $_GET['action'] == "2") && (isset($_GET['t']) && $_GET['t'] == "a")) {
	if (isset($_POST['publish']) && (isset($_GET['submit_id']) && isnum($_GET['submit_id']))) {
		...
		if (dbrows($result)) {
			...
			$result = dbquery("INSERT INTO ".DB_ARTICLES." (article_cat, article_subject, article_snippet, article_article, article_breaks, article_name, article_datestamp, article_reads, article_allow_comments, article_allow_ratings) VALUES ('".$article_cat."', '".$article_subject."', '".$article_snippet."', '".$article_body."', 'y', '".$data['user_id']."', '".time()."', '0', '1', '1')");
			$result = dbquery("DELETE FROM ".DB_SUBMISSIONS." WHERE submit_id='".$_GET['submit_id']."'");
		...
		if (dbrows($result)) {
			...
			if (isset($_POST['preview']) && (isset($_GET['submit_id']) && isnum($_GET['submit_id']))) {
				$article_cat = isnum($_POST['article_cat']) ? $_POST['article_cat'] : "0";
				$article_subject = stripinput($_POST['article_subject']);
				$article_snippet = nl2br(parsesmileys(parseubb($_POST['article_snippet'])));
				$article_body = nl2br(parsesmileys(parseubb($_POST['article_body'])));
				$breaks = (isset($_POST['line_breaks']) ? " checked='checked'" : "");
			...
			echo "<td width='90%' class='tbl'><textarea name='article_snippet' cols='60' rows='5' class='textbox' style='width:80%;'>".$article_snippet."</textarea></td>\n";
			echo "</tr>\n";
			echo "<tr>\n<td class='tbl'></td>\n<td class='tbl'>\n";
				echo display_bbcodes("100%", "article_snippet", "publish");
			echo "</td>\n</tr>\n";
			echo "<tr>\n";
			echo "<td valign='top' width='100' class='tbl'>".$locale['548']."</td>\n";
			echo "<td width='90%' class='tbl'><textarea name='article_body' cols='60' rows='10' class='textbox' style='width:80%;'>".$article_body."</textarea></td>\n";
			echo "</tr>\n";
			echo "<tr>\n<td class='tbl'></td>\n<td class='tbl'>\n";
				echo display_bbcodes("100%", "article_body", "publish");
			echo "</td>\n</tr>\n";
			echo "<tr>\n";
			echo "<td align='center' colspan='2' class='tbl1'><br />\n";
	...
	if (isset($_POST['publish']) && (isset($_GET['submit_id']) && isnum($_GET['submit_id']))) {
		...
		if (dbrows($result)) {
			...
			if ($download_image_thumb) {
				...
				if (is_file(DOWNLOADS."submissions/images/".$download_image_thumb)) {
