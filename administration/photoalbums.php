...
if (function_exists('gd_info')) {
	...
	if (isset($_POST['cancel'])) {
		...
		if (dbcount("(photo_id)", DB_PHOTOS, "album_id='".$_GET['album_id']."'")) {
			if (!isset($_POST['confirm_password'])) {
				...
				if (check_admin_pass(isset($_POST['admin_passwd']) ? stripinput($_POST['admin_passwd']) : "")) {
					if (dbcount("(album_id)", DB_PHOTOS, "album_id='".$_GET['album_id']."'")) {
						$result = dbquery("SELECT album_id, photo_id, photo_filename, photo_thumb1, photo_thumb2 FROM ".DB_PHOTOS." WHERE album_id='".$_GET['album_id']."'");
						while ($data = dbarray($result)) {
							if (!SAFEMODE) {
								if (file_exists(PHOTOS."album_".$data['album_id']."/".$data['photo_filename'])) {
									@unlink(PHOTOS."album_".$data['album_id']."/".$data['photo_filename']);
								}
								if (file_exists(PHOTOS."album_".$data['album_id']."/".$data['photo_thumb1'])) {
									if ($data['photo_thumb1']) { @unlink(PHOTOS."album_".$data['album_id']."/".$data['photo_thumb1']); }
								}
								if (file_exists(PHOTOS."album_".$data['album_id']."/".$data['photo_thumb2'])) {
									if ($data['photo_thumb2']) { @unlink(PHOTOS."album_".$data['album_id']."/".$data['photo_thumb2']); }
								}
							} else {
								if (file_exists(PHOTOS.$data['photo_filename'])) {
									@unlink(PHOTOS.$data['photo_filename']);
								}
								if ($data['photo_thumb1'] && file_exists(PHOTOS.$data['photo_thumb1'])) {
									@unlink(PHOTOS.$data['photo_thumb1']);
								}
								if ($data['photo_thumb2'] && file_exists(PHOTOS.$data['photo_thumb1'])) {
									@unlink(PHOTOS.$data['photo_thumb2']); 
								}
							}
							//comments and ratings do not delete fix
							dbquery("DELETE FROM ".DB_COMMENTS." WHERE comment_type='P' AND comment_item_id='".$data['photo_id']."'");
							dbquery("DELETE FROM ".DB_RATINGS." WHERE rating_type='P' AND rating_item_id='".$data['photo_id']."'");
						}
				...
				$result = dbquery("DELETE FROM ".DB_PHOTO_ALBUMS." WHERE album_id='".$_GET['album_id']."'");
					...
					if (!SAFEMODE) {
						if (@file_exists(PHOTOS."album_".$data['album_id']."/")) {
							$images_files = makefilelist(PHOTOS."album_".$data['album_id']."/", ".|..", true, "files");
							if (is_array($images_files)) {
								foreach ($images_files as $file) { 
									if (file_exists(PHOTOS."album_".$data['album_id']."/".$file)) {
										@unlink(PHOTOS."album_".$data['album_id']."/".$file);
									}
								}
							}
			...
			if (!SAFEMODE) {
				if (@file_exists(PHOTOS."album_".$_GET['album_id']."/")) {
					$images_files = makefilelist(PHOTOS."album_".$_GET['album_id']."/", ".|..", true, "files");
					if (is_array($images_files)) {
						foreach ($images_files as $file) { 
							if (file_exists(PHOTOS."album_".$_GET['album_id']."/".$file)) {
								@unlink(PHOTOS."album_".$_GET['album_id']."/".$file);
							}
						}
					}
