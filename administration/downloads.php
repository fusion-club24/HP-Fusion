...
if (!empty($result)) {
	...
	if ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['download_id']) && isnum($_GET['download_id']))) {
		...
		if (dbrows($result)) {
			...
			$result = dbquery("DELETE FROM ".DB_COMMENTS." WHERE comment_item_id='".$_GET['download_id']."' and comment_type='D'");
			$result = dbquery("DELETE FROM ".DB_RATINGS." WHERE rating_item_id='".$_GET['download_id']."' and rating_type='D'");
